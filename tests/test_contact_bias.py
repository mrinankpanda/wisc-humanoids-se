...
def test_free_fall_with_zero_bias_matches_base():
    ...
    np.testing.assert_allclose(
        v_f, g * t_total, atol=1e-3, err_msg="Velocity doesn't match free-fall"
    )
    np.testing.assert_allclose(
        p_f, 0.5 * g * t_total**2, atol=1e-3, err_msg="Position doesn't match free-fall"
    )
    np.testing.assert_allclose(
        R_f, np.eye(3), atol=1e-10, err_msg="Rotation should stay identity"
    )
...
def test_bias_converges_stationary_robot():
    ...
    np.testing.assert_allclose(
        state.b_L, b_true, atol=5e-3, err_msg="b_hat failed to converge to b_true"
    )
...
def test_reset_contact_clears_cross_correlations():
    ...
    for idx in (IDX_DL, IDX_BL):
        ...
        row[:, idx] = 0
        np.testing.assert_allclose(
            row, 0, err_msg="Off-block row should be zero after reset"
        )
        col = state.P[:, idx].copy()
        col[idx, :] = 0
        np.testing.assert_allclose(
            col, 0, err_msg="Off-block col should be zero after reset"
        )
...
def test_weak_observability_single_support_grows_cov():
    ...
    trace_after = np.trace(state.P[IDX_DL, IDX_DL]) + np.trace(state.P[IDX_BL, IDX_BL])
...
def test_rotation_stays_orthogonal_under_updates():
    ...
    R_final = state.X[:3, :3]
    np.testing.assert_allclose(
        R_final @ R_final.T, np.eye(3), atol=1e-8, err_msg="R should stay orthogonal"
    )
    np.testing.assert_allclose(
        np.linalg.det(R_final), 1.0, atol=1e-8, err_msg="R should have det = 1"
    )
